...
do
      ...
      #S = Simple
      DHT22=$(nice --20 /usr/local/bin/Seeed_DHT22 $WX_TEMP_GPIO S)
      if [[ -n $DHT22 ]]
      then
...
then
  ...
  loglocal "$DATE2" DHT22 ERROR "Error Reading GPIO $WX_TEMP_GPIO"
fi

if test $COUNTER -gt 5
then
  #echo "$DATE2--WARNING-Failed reading GPIO:$HIVE_TEMP_GPIO: retried $COUNTER times" >> $LOG
  loglocal "$DATE2" DHT22 WARNING "Failed reading GPIO $WX_TEMP_GPIO: Retried $COUNTER times"
fi
