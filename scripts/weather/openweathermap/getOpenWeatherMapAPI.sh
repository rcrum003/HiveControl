...
#!/bin/bash
# Version 6
# Script to output OpenWeatherMap.net to a JSON and XML file that matches WUNDERGROUND file format
#
# Revision 1
# 2018-01-15
# 
# 	
#Specific Device
#'https://api.ambientweather.net/v1/devices/B4:E6:2D:07:87:49?apiKey=921a16ba268c40de86bd014246476a536fed326ffb9c41fda8b17ca0cab977af&applicationKey=99d38400a7ff405a9389b113423f3416440cf4e7738841709862a048d7faaa8a&limit=1'

#All Devices
#"https://api.ambientweather.net/v1/devices?applicationKey=99d38400a7ff405a9389b113423f3416440cf4e7738841709862a048d7faaa8a&apiKey=921a16ba268c40de86bd014246476a536fed326ffb9c41fda8b17ca0cab977af"
...
do
	...
	CHECKERROR=$(/bin/echo $GETDATA | JSON.sh -b | grep tempf |awk '{print $2}')
...
DATEUTC=$(/bin/echo $GETDATA | JSON.sh -b | grep dateutc |awk '{print $2}')
WIND_DIR=$(/bin/echo $GETDATA | JSON.sh -b | grep winddir |awk '{print $2}')
WIND_SPEED_MPH=$(/bin/echo $GETDATA | JSON.sh -b | grep windspeedmph |awk '{print $2}')
WIND_GUST_MPH=$(/bin/echo $GETDATA | JSON.sh -b | grep windgustmph |awk '{print $2}')
WIND_MAX_DAILY_GUST=$(/bin/echo $GETDATA | JSON.sh -b | grep windgustmph |awk '{print $2}')
TEMP_F=$(/bin/echo $GETDATA | JSON.sh -b | grep tempf |awk '{print $2}')
RAIN_HOURLY_IN=$(/bin/echo $GETDATA | JSON.sh -b | grep hourlyrainin |awk '{print $2}')
RAIN_EVENT_IN=$(/bin/echo $GETDATA | JSON.sh -b | grep eventrainin |awk '{print $2}')
RAIN_DAILY_IN=$(/bin/echo $GETDATA | JSON.sh -b | grep dailyrainin |awk '{print $2}')
BAROM_REL_IN=$(/bin/echo $GETDATA | JSON.sh -b | grep baromrelin |awk '{print $2}')
BAROM_ABS_IN=$(/bin/echo $GETDATA | JSON.sh -b | grep baromabsin |awk '{print $2}')
HUMIDITY=$(/bin/echo $GETDATA | JSON.sh -b | grep humidity |grep -v humidityin |awk '{print $2}')
UV=$(/bin/echo $GETDATA | JSON.sh -b | grep uv |awk '{print $2}')
SOLARRADIATION=$(/bin/echo $GETDATA | JSON.sh -b | grep solarradiation |awk '{print $2}')
DEWPOINT_F=$(/bin/echo $GETDATA | JSON.sh -b | grep -i dewPoint |awk '{print $2}')
DATE_NORM=$(/bin/echo $GETDATA | JSON.sh -b | grep "date" |grep -v "dateutc" |awk '{print $2}')
