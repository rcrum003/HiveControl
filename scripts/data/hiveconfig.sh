		...
		if [ "$HIVE_CONFIG_VERSION" -gt "$DBVERSION" ]; then # 4.
			...
			/usr/bin/curl --silent --retry 5 -X POST -H "Content-Type: multipart/form-data" -H "X-Requested-With: XMLHttpRequest" -F "api_token=$HIVEAPI" -F "hive_id=$HIVEID" -F "hivename=$HIVENAME" "$POST_GETCONFIG_URL" -o $HIVE_CONFIG
	 		...
	 		#Assign JSON variables to BASH variables
					...
					POST_weather_fallback=$(jq -r .weather_fallback $HIVE_CONFIG)
					POST_weather_fallback_2=$(jq -r .weather_fallback_2 $HIVE_CONFIG)
					POST_wx_max_stale_minutes=$(jq -r .wx_max_stale_minutes $HIVE_CONFIG)
					POST_key_openweathermap=$(jq -r .key_openweathermap $HIVE_CONFIG)
					POST_key_weatherapi=$(jq -r .key_weatherapi $HIVE_CONFIG)
					POST_key_visualcrossing=$(jq -r .key_visualcrossing $HIVE_CONFIG)
					POST_key_pirateweather=$(jq -r .key_pirateweather $HIVE_CONFIG)
					...
					POST_key_airnow=$(jq -r .key_airnow $HIVE_CONFIG)
					POST_enable_airnow=$(jq -r .enable_airnow $HIVE_CONFIG)
					POST_airnow_distance=$(jq -r .airnow_distance $HIVE_CONFIG)
			...
			A POST_weather_fallback WEATHER_FALLBACK
			A POST_weather_fallback_2 WEATHER_FALLBACK_2
			A POST_wx_max_stale_minutes WX_MAX_STALE_MINUTES
			A POST_key_openweathermap KEY_OPENWEATHERMAP
			A POST_key_weatherapi KEY_WEATHERAPI
			A POST_key_visualcrossing KEY_VISUALCROSSING
			A POST_key_pirateweather KEY_PIRATEWEATHER
			A POST_key_tomorrow KEY_TOMORROW
			A POST_key_ambee KEY_AMBEE
			...
			A POST_key_airnow KEY_AIRNOW
			A POST_enable_airnow ENABLE_AIRNOW
			A POST_airnow_distance AIRNOW_DISTANCE
			...
			#Make sure all values have a value
				...
				check_if_blank POST_weather_fallback
				check_if_blank POST_weather_fallback_2
				check_if_blank POST_wx_max_stale_minutes
				check_if_blank POST_key_openweathermap
				check_if_blank POST_key_weatherapi
				check_if_blank POST_key_visualcrossing
				check_if_blank POST_key_pirateweather
