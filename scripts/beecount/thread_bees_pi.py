...
try :
    while True:
        ...
        if bgm.ready :
            ...
            #cv2.imencode("/dev/shm/beecount/foo.jpg", frame[cv2.IMWRITE_JPEG_QUALITY, 90])
            cv2.imwrite("/dev/shm/beecount/frame.jpg", frame)
            fout = open('/dev/shm/beecount/myfile','w')
