...
do
      ...
      #S = Simple
      DHT21=$(nice --20 /usr/local/bin/Seeed_DHT21 $HIVE_TEMP_GPIO S)
      if [[ -n $DHT21 ]]
      then
...
then
  ...
  loglocal "$DATE2" TEMP ERROR "DHT21 Error Reading GPIO $HIVE_TEMP_GPIO"
fi

if test $COUNTER -gt 5
then
  #echo "$DATE2--WARNING-Failed reading GPIO:$HIVE_TEMP_GPIO: retried $COUNTER times" >> $LOG
  loglocal "$DATE2" TEMP WARNING "DHT21 Failed reading GPIO $HIVE_TEMP_GPIO: Retried $COUNTER times"
fi
